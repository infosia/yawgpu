...
| `subgroup-size-control` feature | Not advertised on GLES (Block 108): `GlesAdapter::subgroup_size_control_caps()` returns `None`, so core never inserts the feature and `enable subgroup_size_control;` is rejected at `createShaderModule` exactly as on any other adapter without it. The `HalDevice::create_compute_pipeline` GLES arm returns `HalError` on `required_subgroup_size: Some(_)` (unreachable by validation). No GLES 3.1 analog for a required subgroup width. | locked ✗ |
