...
| ANGLE platform selection | Leave ANGLE to its default backend choice (typically D3D11 on Windows). Do not expose `EGL_ANGLE_platform_angle` controls from yawgpu. | Keeps the surface simple; users wanting a specific ANGLE backend can env-var their own ANGLE build. |
...
| Adapter selection from `wgpuCreateInstance` | Default backend priority unchanged (`Vulkan > Metal > Noop`). GLES is selected **only** when the caller explicitly requests `WGPUBackendType_OpenGLES` via `WGPURequestAdapterOptions.backendType` (the yawgpu.h `YaWGPUInstanceBackendSelect` vendor extension is **not** extended for GLES). Implemented in P15.1a; `wgpuInstanceRequestAdapter` routes `OpenGLES` requests to a side `gles_core` instance probed at `wgpuCreateInstance` time, falling back to `WGPURequestAdapterStatus_Unavailable` when EGL/ANGLE init fails. | Prevents accidental Tier 2 selection; keeps yawgpu.h surface untouched. |
...
| Render pass (color + depth/stencil) | Transient FBO + `glFramebufferTexture2D(COLOR_ATTACHMENT0)` + `glDrawBuffers` + `glViewport` + clear (`glClearColor`/`glClear`); `RenderPassCleanup` Drop guard ensures VAO + FBO + program-state + memory-barrier cleanup runs regardless of inner error. Clear-only render passes (`pass.pipeline == None`) take the FBO+clear path and skip VAO+draw (Q1 fix) | ◐ (P15.5 + Phase 15 Review: color-only including clear-only passes; depth/stencil deferred) |
