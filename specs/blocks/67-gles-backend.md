...
| Error mapping | `HalError::BackendOperationFailed { backend: "gles", message }` for GL/EGL failures; specific variants (`AcquireFailed`, `PresentFailed`, `SwapchainCreationFailed`, …) where the existing enum has them. | Mirrors Vk / Metal arms. |
| Adapter selection from `wgpuCreateInstance` | Default backend priority unchanged (`Vulkan > Metal > Noop`). GLES is selected **only** when the caller explicitly requests `WGPUBackendType_OpenGLES`. | Prevents accidental Tier 2 selection. |
