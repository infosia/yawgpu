...
| `textureNumLevels` / `textureNumSamples` (`texture_builtins_from_uniform`) | Not wired: the identity-remap path bypasses `glsl::writer::GenerateBindings`, so the texture-builtin polyfill UBO defaults to binding `{0,0}` and could alias a user buffer at binding 0 in the emitted GLSL. Unreachable today — any texture binding in a GLES pass already returns `HalError` before a draw/dispatch — but must be wired (explicit polyfill binding via `Options::texture_builtins_from_uniform`) when GLES texture-binding support lands. | ✗ (Phase Review m5 catalogue entry; blocked on GLES texture bindings) |
