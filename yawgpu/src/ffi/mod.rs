...
impl WGPUDeviceImpl {
    ...
    fn schedule_device_lost(
        ...
        let reason = if reason == core::DeviceLostReason::Destroyed {
            self.core.destroy()?
        } else {
            self.core.lose(reason)?
        };
...
mod tests {
    ...
    #[cfg(feature = "vulkan")]
    #[test]
    #[ignore = "manual real Vulkan regression test"]
    fn wgpuDeviceDestroy_reclaims_vulkan_device_during_compute_pipeline_churn() {
        const ITERATIONS: usize = 160;
        const STORAGE_PIPELINE_WGSL: &str = r"
        ...
        unsafe {
            ...
            for iteration in 0..ITERATIONS {
                ...
                assert!(
                    !instance.is_null(),
                    "iteration {iteration}: Vulkan instance creation failed"
                );

                let mut adapter_state = RequestAdapterState::default();
                let adapter_callback_info = native::WGPURequestAdapterCallbackInfo {
                    nextInChain: std::ptr::null_mut(),
                    mode: native::WGPUCallbackMode_AllowProcessEvents,
                    callback: Some(request_adapter_callback),
                    userdata1: (&mut adapter_state as *mut RequestAdapterState).cast(),
                    userdata2: std::ptr::null_mut(),
                };
                let future =
                ...
                assert_eq!(
                    ...
                    native::WGPURequestAdapterStatus_Success,
                    "iteration {iteration}: requestAdapter failed"
                );
                assert!(
                    !adapter_state.adapter.is_null(),
                    "iteration {iteration}: requestAdapter returned null"
                );

                let mut device_state = RequestDeviceState::default();
                let device_callback_info = native::WGPURequestDeviceCallbackInfo {
                    nextInChain: std::ptr::null_mut(),
                    mode: native::WGPUCallbackMode_AllowProcessEvents,
                    callback: Some(request_device_callback),
                    userdata1: (&mut device_state as *mut RequestDeviceState).cast(),
                    userdata2: std::ptr::null_mut(),
                };
                ...
                assert_ne!(future.id, 0);
                for _ in 0..8 {
                    if device_state.fired != 0 {
                        break;
                    }
                    wgpuInstanceProcessEvents(instance);
                }
                if device_state.status != native::WGPURequestDeviceStatus_Success
                    || device_state.device.is_null()
                {
                    wgpuAdapterRelease(adapter_state.adapter);
                    wgpuInstanceRelease(instance);
                    panic!(
                        "iteration {iteration}: requestDevice failed with status {:?}",
                        device_state.status
                    );
                }
                ...
                let bgl = wgpuDeviceCreateBindGroupLayout(device_state.device, &bgl_descriptor);
                assert!(
                    !bgl.is_null(),
                    "iteration {iteration}: bind group layout creation returned null"
                );

                let layouts = [bgl];
                let pipeline_layout_desc = pipeline_layout_descriptor(&layouts);
                let pipeline_layout = wgpuDeviceCreatePipelineLayout(
                    device_state.device,
                    &pipeline_layout_desc,
                );
                assert!(
                    !pipeline_layout.is_null(),
                    "iteration {iteration}: pipeline layout creation returned null"
                );

                let module = create_wgsl_module(device_state.device, STORAGE_PIPELINE_WGSL);
                let pipeline_descriptor = compute_pipeline_descriptor(module, pipeline_layout);
                let pipeline =
                    wgpuDeviceCreateComputePipeline(device_state.device, &pipeline_descriptor);
                assert!(
                    !pipeline.is_null(),
                    "iteration {iteration}: compute pipeline creation returned null"
                );
                assert!(
                    !borrow_handle(pipeline, "WGPUComputePipeline")
                        ._core
                        .is_error(),
                    "iteration {iteration}: compute pipeline is an error pipeline"
                );

                wgpuDeviceDestroy(device_state.device);
