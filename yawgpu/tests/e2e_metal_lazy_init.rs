...
fn lazy_init_compressed_mip_copies_zero_bytes_and_canary_mip_survives() {
    ...
    unsafe {
        let s = Session::with_features(&[native::WGPUFeatureName_TextureCompressionBC]);
        if s.device.is_null() {
...
impl Session {
    ...
    unsafe fn with_features(features: &[native::WGPUFeatureName]) -> Self {
