//! Real-GPU verification of the WebGPU `depth-clip-control` feature on Vulkan.
//!
...
#[cfg(feature = "vulkan")]
fn vulkan_adapter_advertises_depth_clip_control() {
    ...
    unsafe {
        ...
        assert!(
            ...
            "Vulkan adapter must advertise depth-clip-control"
        );
...
#[ignore = "manual real-backend test"]
#[cfg(feature = "vulkan")]
fn vulkan_unclipped_depth_draws_beyond_far_plane() {
    if real_backend_skip_reason(RealBackend::Vulkan).is_some() {
        return;
    }
    unsafe {
