//! Real-GPU verification of the WebGPU `clip-distances` feature on Metal.
//!
...
//! clip-distance actually culls, i.e. Tint's `[[clip_distance]]` MSL output is
//! reached through the new feature. Uses `enable clip_distances;`, only accepted
...
#[cfg(feature = "metal")]
fn metal_adapter_advertises_clip_distances() {
    ...
    unsafe {
        ...
        assert!(
            ...
            "Metal adapter must advertise clip-distances"
        );
...
#[ignore = "manual real-backend test"]
#[cfg(feature = "metal")]
fn metal_clip_distances_cull_by_sign() {
    if real_backend_skip_reason(RealBackend::Metal).is_some() {
        return;
    }
    unsafe {
        ...
        assert!(
            ...
            "clip-distances path raised a device error"
        );
