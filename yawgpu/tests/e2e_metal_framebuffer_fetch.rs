//! Real-Metal e2e for `@color(N)` framebuffer fetch (TBDR re-home, Slice 1.3).
//!
//! A fragment shader reads the current color-attachment value via
//! `@color(0)` (Metal programmable blending / `[[color(0)]]`) and adds red.
//! The attachment is cleared to (0.1, 0.2, 0.3, 1.0); on the triangle the
//! fragment must observe that cleared value and produce (0.6, 0.2, 0.3, 1.0).
//! Verifying the read requires real GPU execution + readback.
//!
//! Gated on `metal` + `tiled`; run with:
//!   cargo test -p yawgpu --features metal,tiled --test e2e_metal_framebuffer_fetch -- --ignored
...
fn metal_framebuffer_fetch_reads_cleared_attachment() {
    ...
    unsafe {
        ...
        // read the cleared attachment value via @color(0).
        assert!(
