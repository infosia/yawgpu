//! Real-GPU verification of the WebGPU `subgroups` feature on Vulkan.
//!
//! Proves the feature end-to-end on hardware (not just Noop validation):
//! - the Vulkan adapter advertises `WGPUFeatureName_Subgroups` and reports a
...
//!   subgroup op (`subgroupAdd`) — readback-verified (exercises SPIR-V
//!   `GroupNonUniform*` + the subgroup builtins);
...
//!
//! Runs against native Vulkan or MoltenVK. MoltenVK maps subgroup ops onto
//! Metal SIMD-group functions; the arithmetic subset used here is supported.
#![cfg(feature = "vulkan")]
...
#[cfg(feature = "vulkan")]
fn vulkan_adapter_advertises_subgroups() {
    ...
    unsafe {
        ...
        assert!(
            ...
            "Vulkan adapter must advertise subgroups"
        );
...
#[ignore = "manual real-backend test"]
#[cfg(feature = "vulkan")]
fn vulkan_subgroup_size_and_reduction() {
    if real_backend_skip_reason(RealBackend::Vulkan).is_some() {
        return;
    }
    unsafe {
...
#[ignore = "manual real-backend test"]
#[cfg(feature = "vulkan")]
fn vulkan_subgroups_shader_rejected_without_feature() {
    if real_backend_skip_reason(RealBackend::Vulkan).is_some() {
        return;
    }
    unsafe {
