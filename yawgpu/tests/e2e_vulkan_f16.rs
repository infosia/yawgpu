//! Real-GPU verification of the WebGPU `shader-f16` feature on Vulkan
//! (MoltenVK on Apple Silicon, and native desktop Vulkan).
//!
//! Exercises the Vulkan device-feature enablement path
//! (`VK_KHR_shader_float16_int8` `shaderFloat16` + `VK_KHR_16bit_storage`
//! `storageBuffer16BitAccess`): an `enable f16;` compute shader that reads f16
//! from a storage buffer, does f16 arithmetic, and writes f16 back —
//! readback-verified. Plus adapter-advertise and the without-feature S12 gate.
#![cfg(feature = "vulkan")]
...
#[cfg(feature = "vulkan")]
fn vulkan_adapter_advertises_shader_f16() {
...
#[ignore = "manual real-backend test"]
#[cfg(feature = "vulkan")]
fn vulkan_f16_compute_doubles_values() {
    if real_backend_skip_reason(RealBackend::Vulkan).is_some() {
        return;
    }
    unsafe {
...
"#;
        ...
        let out_size = (ELEMENTS * 2) as u64;
