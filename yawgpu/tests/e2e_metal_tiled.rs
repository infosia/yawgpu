...
//
// The subpass path previously dropped setViewport/setScissorRect (group B, both
// backends) and — on Metal — cullMode/frontFace (group A; these are Metal render-
// encoder state the subpass encode skipped). Each probe runs a SINGLE-subpass pass
// that clears the persistent color target to RED then draws an oversized CCW green
// triangle; the read-back center (8,8) / corner (0,0) distinguish the GPU effect.

// Oversized triangle (CCW in WebGPU NDC), green fragment.
...
#[test]
#[ignore = "manual real-backend test"]
fn metal_tiled_subpass_scissor_restricts_rendering() {
...
#[test]
#[ignore = "manual real-backend test"]
fn metal_tiled_subpass_viewport_restricts_rendering() {
    run_subpass_region_check(SubpassRegion::Viewport);
}

/// Metal tiled subpass: `cull=Back, frontFace=CCW` keeps the front-facing draw.
#[test]
#[ignore = "manual real-backend test"]
fn metal_tiled_subpass_cull_back_keeps_front_facing_ccw() {
...
/// Metal tiled subpass: `cull=Back, frontFace=CW` culls the (now back-facing) draw
/// (regression check — the subpass encode previously never set cull/winding).
#[test]
#[ignore = "manual real-backend test"]
fn metal_tiled_subpass_cull_back_discards_back_facing_cw() {
    let pixels = run_tiled_subpass(
