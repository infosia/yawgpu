//! Real-GPU verification of the WebGPU `float32-blendable` feature on Metal.
//!
...
#[cfg(feature = "metal")]
fn metal_adapter_advertises_float32_blendable() {
    ...
    unsafe {
        ...
        assert!(
            ...
            "Metal adapter must advertise float32-blendable"
        );
...
#[ignore = "manual real-backend test"]
#[cfg(feature = "metal")]
fn metal_float32_target_blends_additively() {
    if real_backend_skip_reason(RealBackend::Metal).is_some() {
        return;
    }
    unsafe {
