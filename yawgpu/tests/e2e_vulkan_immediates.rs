//! Real-Vulkan (MoltenVK on this Mac) e2e for Block 94 immediates: `wgpu*SetImmediates` user data
...
#[cfg(feature = "vulkan")]
fn vulkan_compute_immediates_reach_shader() {
...
#[ignore = "manual real-backend test"]
#[cfg(feature = "vulkan")]
fn vulkan_compute_immediates_partial_update_between_dispatches() {
    if real_backend_skip_reason(RealBackend::Vulkan).is_some() {
        return;
    }
...
#[ignore = "manual real-backend test"]
#[cfg(feature = "vulkan")]
fn vulkan_frag_depth_from_immediate_composes_with_clamp() {
    if real_backend_skip_reason(RealBackend::Vulkan).is_some() {
        return;
    }
