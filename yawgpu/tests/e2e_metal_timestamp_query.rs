//! Real-Metal e2e for Block 102 (`specs/blocks/102-timestamp-query.md`):
...
const BACKEND: RealBackend = RealBackend::Metal;
...
fn metal_write_timestamp_around_a_dispatch_resolves_increasing_nanoseconds() {
...
#[test]
#[ignore = "manual real-backend test"]
fn metal_pass_timestamp_writes_with_only_an_end_index_leave_the_other_slot_zero() {
    if real_backend_skip_reason(BACKEND).is_some() {
        return;
    }
    unsafe {
...
#[test]
#[ignore = "manual real-backend test"]
fn metal_resolving_a_never_written_range_yields_zeros() {
    if real_backend_skip_reason(BACKEND).is_some() {
        return;
    }
    unsafe {
...
#[test]
#[ignore = "manual real-backend test"]
fn metal_two_sets_resolved_into_one_buffer_at_different_offsets_do_not_clobber() {
    if real_backend_skip_reason(BACKEND).is_some() {
        return;
    }
    unsafe {
