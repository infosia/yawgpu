//! Real-GPU Metal verification for the threading audit: WebGPU state fields that
//! `yawgpu-core` validated but never threaded to the HAL, so the GPU used a
//! default and the result was silently wrong. Each probe sets a NON-default
//! value and reads back the GPU effect that only appears once the field reaches
//! the backend:
//!
...
//!   `cull=Back, frontFace=CW` (now back → discarded). The Vulkan mirror
//!   (`e2e_vulkan_threading_audit`) pins that the SAME pipeline culls identically
//!   on both backends (Y/winding parity).
...
//!
//! Gated on the `metal` feature; `#[ignore]`d (manual real-backend run).
#![cfg(feature = "metal")]
...
fn metal_cull_back_keeps_front_facing_ccw() {
...
#[test]
#[ignore = "manual real-backend test"]
fn metal_cull_back_discards_back_facing_cw() {
...
#[test]
#[ignore = "manual real-backend test"]
fn metal_scissor_rect_restricts_rendering() {
...
#[test]
#[ignore = "manual real-backend test"]
fn metal_viewport_restricts_rendering() {
...
#[test]
#[ignore = "manual real-backend test"]
fn metal_read_only_depth_preserves_contents() {
...
#[test]
#[ignore = "manual real-backend test"]
fn metal_dispatch_workgroups_indirect_executes() {
...
/// Narrowing audit: `loadOp=Clear` of an integer (R32Uint) render target must
/// store the EXACT integer clear value. The pre-fix Vulkan path wrote the
/// `float32` member of `VkClearColorValue` regardless of format, so an integer
/// target got a float-bit-reinterpreted garbage value. Metal was always correct;
/// this is the cross-backend parity probe (mirror runs on Vulkan/MoltenVK).
#[test]
#[ignore = "manual real-backend test"]
fn metal_integer_render_target_clear_is_exact() {
...
/// Two draws in ONE render pass (loadOp=Clear) must ACCUMULATE: the first draw's
/// output must survive the second. Probe: clear red, draw1 scissored to the left
/// half (green), draw2 scissored to the right half (green) — both halves must end
/// green. If the per-draw replay re-clears between draws, the left half reverts to
/// red (only the last draw survives). Audit A execution-model check.
#[test]
#[ignore = "manual real-backend test"]
fn metal_two_draws_one_pass_accumulate() {
