...
fn vulkan_tiled_features_and_capabilities_are_advertised() {
    if vulkan_backend_skip_reason().is_some() {
        return;
    }
...
#[test]
#[ignore = "manual real-backend test"]
fn vulkan_two_subpass_draw_subpass_load_readback() {
    if vulkan_backend_skip_reason().is_some() {
        return;
    }
    ...
    unsafe {
        let instance = create_vulkan_instance();
        let adapter = request_adapter(instance);
        if adapter_is_moltenvk(adapter) {
            eprintln!(
                "skipping Vulkan subpass-input read e2e on MoltenVK; requires a native Vulkan driver"
            );
            yawgpu::wgpuAdapterRelease(adapter);
            yawgpu::wgpuInstanceRelease(instance);
            return;
...
fn vulkan_backend_skip_reason() -> Option<String> {
    suppress_moltenvk_probe_logs();
    real_backend_skip_reason(RealBackend::Vulkan)
}

fn suppress_moltenvk_probe_logs() {
    static INIT: Once = Once::new();
    INIT.call_once(|| {
        std::env::set_var("MVK_CONFIG_LOG_LEVEL", "0");
    });
}
...
unsafe fn adapter_is_moltenvk(adapter: native::WGPUAdapter) -> bool {
    ...
    let vendor = string_view_to_string(info.vendor);
    let device = string_view_to_string(info.device);
    let description = string_view_to_string(info.description);
