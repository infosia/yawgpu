//! Real-Vulkan regression for F-065 out-of-memory classification.
//!
...
//! never as a validation error and never as a panic.
//!
//! The descriptor used here is a 4096 x 4096 x 256 `rgba32float` 2D-array
//! texture (= 64 GiB). 4096 == `maxTextureDimension2D` and 256 ==
//! `maxTextureArrayLayers` (the yawgpu default limits, which the Vulkan adapter
//! also advertises), so core validation accepts it and it reaches the Vulkan
//! HAL, where `vkCreateImage` / `vkAllocateMemory` returns
//! `ERROR_OUT_OF_DEVICE_MEMORY` / `ERROR_OUT_OF_HOST_MEMORY` → mapped to
...
#[cfg(feature = "vulkan")]
fn vulkan_oversized_texture_reports_out_of_memory_without_panic() {
    ...
    unsafe {
        ...
        // error; the Vulkan allocation failure is routed as GPUOutOfMemoryError.
        ...
        // Reaching past this call at all proves the Vulkan HAL did not panic on
        // the allocation failure (it returns Err(OutOfMemory) instead).
        ...
        assert!(
            ...
            "expected a GPUOutOfMemoryError for the oversized Vulkan texture, got: {captured:?}"
        );
        assert!(
            captured
                .iter()
