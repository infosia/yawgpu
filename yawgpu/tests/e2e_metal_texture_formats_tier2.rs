//! Real-Metal e2e for Block 72 (`specs/blocks/72-texture-formats-tier2.md`):
//! `texture-formats-tier2` is advertised iff `MTLDevice.readWriteTextureSupport`
//! reports tier 2 (Dawn's rule), a device without the feature rejects a
//! `read-write` `rgba8unorm` storage-texture layout with a message naming the
//! format and the tier, and a device with the feature actually executes a
//! `texture_storage_2d<rgba8unorm, read_write>` compute pass.
...
use objc2_metal::{MTLCreateSystemDefaultDevice, MTLDevice, MTLReadWriteTextureTier};
use yawgpu::native;
use yawgpu::{
...
fn initial_pixels() -> Vec<u8> {
    let mut pixels = Vec::with_capacity(ROW_BYTES * HEIGHT as usize);
    for y in 0..HEIGHT as u8 {
        for x in 0..WIDTH as u8 {
            pixels.extend_from_slice(&[x * 80 + 10, y * 80 + 20, 0, 255]);
        }
    }
    pixels
}
...
/// Dawn's advertisement rule, queried straight from Metal as the oracle.
fn metal_device_reports_read_write_tier2() -> bool {
    let device = MTLCreateSystemDefaultDevice().expect("system default Metal device");
    device.readWriteTextureSupport() == MTLReadWriteTextureTier::Tier2
}

#[test]
#[ignore = "manual real-backend test"]
fn metal_adapter_advertises_tier2_iff_device_reports_read_write_tier2() {
    ...
    unsafe {
        ...
        let advertised =
            yawgpu::wgpuAdapterHasFeature(adapter, native::WGPUFeatureName_TextureFormatsTier2)
                != 0;
        assert_eq!(
            advertised,
            metal_device_reports_read_write_tier2(),
            "texture-formats-tier2 advertisement must follow MTLDevice.readWriteTextureSupport"
        );
...
#[test]
#[ignore = "manual real-backend test"]
fn metal_device_without_tier2_rejects_rgba8unorm_read_write_layout() {
    if real_backend_skip_reason(RealBackend::Metal).is_some() {
        return;
    }
    ...
    unsafe {
        ...
        let bgl = create_read_write_rgba8unorm_bgl(device);
...
#[test]
#[ignore = "manual real-backend test"]
fn metal_device_with_tier2_executes_rgba8unorm_read_write_storage() {
    if real_backend_skip_reason(RealBackend::Metal).is_some() {
        return;
    }
    if !metal_device_reports_read_write_tier2() {
        eprintln!("skipping: this Metal device is below read-write texture tier 2");
        return;
    }
    ...
    unsafe {
        ...
        let texture = create_rgba8unorm_storage_texture(device);
        write_texture_pixels(queue, texture, &initial_pixels());
        ...
        let module = create_wgsl_module(device, RGBA8_READ_WRITE_SHADER);
        ...
        let actual = read_unpadded_pixels(instance, readback);
        assert_eq!(
            actual,
            expected_pixels(),
            "rgba8unorm read-write storage texture did not round-trip through the compute pass"
        );
        assert!(
...
unsafe fn create_read_write_rgba8unorm_bgl(
...
unsafe fn create_rgba8unorm_storage_texture(device: native::WGPUDevice) -> native::WGPUTexture {
