//! Real-GPU verification of the WebGPU `dual-source-blending` feature on Metal.
//!
...
#[cfg(feature = "metal")]
fn metal_adapter_advertises_dual_source_blending() {
    ...
    unsafe {
        ...
        assert!(
            yawgpu::wgpuAdapterHasFeature(adapter, native::WGPUFeatureName_DualSourceBlending) != 0,
            "Metal adapter must advertise dual-source-blending"
        );
...
#[ignore = "manual real-backend test"]
#[cfg(feature = "metal")]
fn metal_dual_source_blend_multiplies_two_outputs() {
    if real_backend_skip_reason(RealBackend::Metal).is_some() {
        return;
    }
    unsafe {
