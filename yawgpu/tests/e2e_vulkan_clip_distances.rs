//! Real-GPU verification of the WebGPU `clip-distances` feature on Vulkan.
//!
//! A full-screen triangle sets a single `@builtin(clip_distances)` value on every
//! vertex: **negative** clips every fragment (nothing drawn → clear color), and
//! **positive** keeps them (drawn red). Reading back the two targets proves the
//! clip-distance actually culls through the Vulkan backend.
//! Uses `enable clip_distances;`, only accepted
//! when the device requested the feature.
#![cfg(feature = "vulkan")]
...
fn shader(clip_value: &str) -> String {
    format!(
...
#[cfg(feature = "vulkan")]
fn vulkan_adapter_advertises_clip_distances() {
    ...
    unsafe {
        ...
        if yawgpu::wgpuAdapterHasFeature(adapter, native::WGPUFeatureName_ClipDistances) == 0 {
            eprintln!("Skipping: Vulkan adapter does not advertise clip-distances");
            yawgpu::wgpuAdapterRelease(adapter);
            yawgpu::wgpuInstanceRelease(instance);
            return;
        }
        assert!(
            yawgpu::wgpuAdapterHasFeature(adapter, native::WGPUFeatureName_ClipDistances) != 0,
            "Vulkan adapter must advertise clip-distances"
        );
        println!("Vulkan adapter clip-distances: true");
        yawgpu::wgpuAdapterRelease(adapter);
        yawgpu::wgpuInstanceRelease(instance);
    }
}
...
#[ignore = "manual real-backend test"]
#[cfg(feature = "vulkan")]
fn vulkan_clip_distances_cull_by_sign() {
    if let Some(reason) = real_backend_skip_reason(RealBackend::Vulkan) {
        eprintln!("Skipping Vulkan clip-distances: {reason}");
        return;
    }
    ...
    unsafe {
        ...
        let device = request_device_with_clip_distances(instance, adapter);
        assert!(
            yawgpu::wgpuDeviceHasFeature(device, native::WGPUFeatureName_ClipDistances) != 0,
            "device must report clip-distances after requesting it"
        );
        let errors = install_error_capture(device);
        let queue = yawgpu::wgpuDeviceGetQueue(device);

        let clipped = render_first_pixel(instance, device, queue, &shader("-1.0"));
        println!("clip -1.0: {clipped:?}");
        assert_eq!(
            clipped,
            [0, 0, 0, 255],
            "a negative clip distance must cull the whole triangle (clear color)"
        );

        let kept = render_first_pixel(instance, device, queue, &shader("1.0"));
        println!("clip +1.0: {kept:?}");
        assert_eq!(
            kept,
            [255, 0, 0, 255],
            "a positive clip distance must keep the triangle (drawn red)"
        );

        assert!(
            errors.lock().expect("error lock").is_empty(),
            "clip-distances path raised a device error: {errors:?}"
        );
...
#[cfg(feature = "vulkan")]
unsafe fn request_device_with_clip_distances(
    instance: native::WGPUInstance,
    adapter: native::WGPUAdapter,
) -> native::WGPUDevice {
    let features = [native::WGPUFeatureName_ClipDistances];
