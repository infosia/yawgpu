//! Real-Metal e2e for Block 100 (`specs/blocks/100-encoder-write-buffer.md`):
...
fn metal_encoder_write_buffer_writes_bytes_at_offset_and_leaves_the_rest_zero() {
...
#[test]
#[ignore = "manual real-backend test"]
fn metal_encoder_write_buffer_then_copy_in_one_encoder_orders_the_write_first() {
    if real_backend_skip_reason(BACKEND).is_some() {
        return;
    }
    unsafe {
...
#[test]
#[ignore = "manual real-backend test"]
fn metal_encoder_write_buffer_larger_than_a_staging_chunk_round_trips() {
    if real_backend_skip_reason(BACKEND).is_some() {
        return;
    }
