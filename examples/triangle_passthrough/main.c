...
// Vulkan, MSL for Metal) via the bundled Tint compiler. That covers
...
//     shader bytecode.
//
// For these cases yawgpu's `shader-passthrough` cargo feature (opt-in,
// default off) lets `wgpuDeviceCreateShaderModule` take native bytes via
// a chained source struct instead of WGSL. It is a vendor escape hatch:
// the bytes reach the driver verbatim with no validation or reflection,
// so the caller owns correctness. Both paths require an EXPLICIT pipeline
// layout (no `layout: "auto"` — there is no reflection to derive one):
//
//   * SPIR-V (Vulkan) — chain the STANDARD `WGPUShaderSourceSPIRV`
//     (`sType = WGPUSType_ShaderSourceSPIRV`) onto
//     `WGPUShaderModuleDescriptor.nextInChain`:
//     - `.code` is a `uint32_t*` array of SPIR-V words (NOT bytes);
//       `.codeSize` is the WORD count.
//     - yawgpu does NOT reflect the SPIR-V; the entry-point name baked
//       into the module must match the pipeline's `entryPoint`.
//     - A `.spv` typically holds one stage, so two files become two
//       `WGPUShaderModule` handles referenced separately.
//
//   * MSL (Metal) — chain the vendor `YaWGPUShaderSourceMSL`
//     (`sType = YAWGPU_STYPE_SHADER_SOURCE_MSL`):
//     - `.code` is a `WGPUStringView` of plain MSL text.
//     - `.entryPoints` is a `YaWGPUMslEntryPoint[]` declaring each
//       function (name, stage, optional compute workgroup size) — the
//       caller enumerates them because yawgpu doesn't reflect MSL either.
...
static WGPUShaderModule create_spirv_module(PassthroughApp *app, const char *name) {
    ...
    // SPIR-V passthrough uses the STANDARD webgpu.h WGPUShaderSourceSPIRV
    // chained struct (no vendor function); yawgpu routes it to its verbatim,
    // no-reflection SPIR-V path when the library is built with the
    // `shader-passthrough` cargo feature. `codeSize` is the WORD count, and the
    // words are copied synchronously so `bytes` can be freed right after.
    WGPUShaderSourceSPIRV spirv = {
        .chain = {.sType = WGPUSType_ShaderSourceSPIRV},
        .codeSize = (uint32_t)(byte_size / sizeof(uint32_t)),
        .code = (const uint32_t *)bytes,
    };
    WGPUShaderModuleDescriptor desc = {
        .nextInChain = &spirv.chain,
        .label = yawgpu_string_view(name),
    };
    WGPUShaderModule module = wgpuDeviceCreateShaderModule(app->context.device, &desc);
    free(bytes);
    if (!module) {
        fprintf(stderr, "wgpuDeviceCreateShaderModule (SPIR-V) failed for %s\n", name);
    }
    return module;
}
...
static WGPUShaderModule create_msl_module(PassthroughApp *app, const char *name) {
    ...
    // MSL passthrough uses the vendor YaWGPUShaderSourceMSL chained struct
    // (Metal-only). yawgpu takes the source verbatim and the explicit
    // entry-point list; it does not reflect the MSL.
    YaWGPUShaderSourceMSL msl = YAWGPU_SHADER_SOURCE_MSL_INIT;
    msl.code = (WGPUStringView){.data = (const char *)bytes, .length = byte_size};
    msl.entryPointCount = 2;
    msl.entryPoints = entries;

    WGPUShaderModuleDescriptor desc = {
        .nextInChain = &msl.chain,
        .label = yawgpu_string_view(name),
    };
    WGPUShaderModule module = wgpuDeviceCreateShaderModule(app->context.device, &desc);
    free(bytes);
    if (!module) {
        fprintf(stderr, "wgpuDeviceCreateShaderModule (MSL) failed for %s\n", name);
    }
    return module;
}
